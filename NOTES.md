...
## Capstone — assembling the service, and a real testing gotcha
Wired `/v1/entities`, `/v1/search`, `/v1/analyse`, and a batch extension `/v1/classify:batch` into `api.py`, all reusing the Lab 5/7 artefacts (INT8 classifier + NER, the Lab 5 FAISS index/reranker).

**Testing gotcha (not a code bug) worth recording**: manually smoke-testing `/v1/entities` and `/v1/search` with `curl -d "{...arabic text...}"` inline on the command line initially looked broken -- entities were missing (only DATE/REFERENCE came back, no SERVICE/LOCATION) and search returned empty results. Traced it by calling the exact same predict function directly in Python (got the correct, full result) before suspecting the server itself. Root cause: Windows console/shell encoding mangles Arabic text passed as an inline command-line argument to `curl` before it ever reaches the server as JSON. Switching to `curl --data-binary @file.json` with the JSON written to a UTF-8 file up front fixed it immediately -- the API code was correct throughout. Lesson: when debugging a "the server is wrong" symptom with non-ASCII test input on Windows, rule out the *test harness's* encoding before the service.

**Real, expected limitation confirmed live**: running `/v1/analyse` on a pothole complaint ("حفرة في الطريق أمام حي العليا منذ 16 أيام" -- a phrasing style from the Lab 5 case corpus, not the narrow Lab 3B/4 NER training templates) mistagged "حي" as SERVICE and only got "العليا" for LOCATION instead of the full "حي العليا" neighbourhood name. This is exactly the gap already flagged by hand in the NER model card ("No adversarial or out-of-template text was evaluated; performance on genuinely novel phrasing is unknown") -- now empirically demonstrated, not just hypothesised. Classification and search on the same input were both excellent (roads, 0.99 confidence; top match case_id CASE-008921, score 0.9986, same underlying complaint template).

**Also reconfirmed live**: the Lab 1 repeated-character normalisation still collapses reference-code digit runs in production (`BYN-000000` → `BYN-00` after `preprocess()`), consistent with the documented Lab 1/4 finding -- any feature needing the exact reference code must read it from raw text, not normalised text.
